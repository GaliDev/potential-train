...
def compare(
    ...
    """Evaluate the baseline and the panel on the same items and compare."""
    configs = {
        "baseline": BaselineJudge(),
        "panel": PanelJudge(),
    }
    out: dict = {"n_items": len(items), "configs": {}}
    for name, judge in configs.items():
...
def main() -> None:
    ...
    parser.add_argument("--mode", choices=["baseline", "panel", "compare"], default="compare")
    ...
    if args.mode == "compare":
        report = compare(items, persist=args.persist)
        for name, m in report["configs"].items():
            _print_metrics(name, m)
    else:
        judge = BaselineJudge() if args.mode == "baseline" else PanelJudge()
